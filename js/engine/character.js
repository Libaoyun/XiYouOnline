...
class Character {
  ...
  drawBody(ctx, sx, sy) {
    if (window.CharacterRenderer) {
      ...
      let modelToDraw = isRiding ? 'mount_knight' : this.appearance;
      if (!modelToDraw) {
        if (this.type === 'player') {
          modelToDraw = 'martial_hero';
        } else if (this.type === 'monster') {
          modelToDraw = this.monsterType || Character.inferMonsterType(this.name, this.id) || 'wild_wolf';
        } else {
          modelToDraw = 'martial_hero';
        }
      }
...
class CharacterRenderer {
  ...
  static drawClam(ctx, by, animTimer, direction) {
    ...
    const flip = direction === 'left' ? -1 : 1;
    ctx.scale(flip, 1);
    const breathe = Math.sin(animTimer * 0.15) * 2;

    // 外壳阴影
    ctx.fillStyle = 'rgba(0, 0, 0, 0.4)';
    ctx.beginPath();
    ctx.ellipse(0, by + 8, 14, 5, 0, 0, Math.PI * 2);
    ctx.fill();

    // 下半壳 (青灰硬壳带纹)
    ctx.fillStyle = '#4a5568';
    ctx.beginPath();
    ctx.ellipse(0, by + 5, 14, 8, 0, 0, Math.PI);
    ctx.fill();
    ...
    ctx.beginPath();
    ctx.ellipse(0, by + 2, 11, 5, 0, 0, Math.PI * 2);
    ctx.fill();

    // 硕大温润发光东海明珠！
    const pearlGlow = Math.sin(animTimer * 0.2) * 2;
    const pGrad = ctx.createRadialGradient(0, by + 1, 1, 0, by + 1, 7 + pearlGlow);
    pGrad.addColorStop(0, '#ffffff');
    pGrad.addColorStop(0.4, '#e0f2fe');
    pGrad.addColorStop(0.8, '#38bdf8');
    pGrad.addColorStop(1, 'rgba(56, 189, 248, 0)');
    ctx.fillStyle = pGrad;
    ctx.beginPath();
    ctx.arc(0, by + 1, 7 + pearlGlow, 0, Math.PI * 2);
    ctx.fill();
    ...
    ctx.beginPath();
    ctx.ellipse(0, by - 2 - breathe, 14, 9, 0, Math.PI, 0);
    ctx.fill();
    ...
    ctx.lineWidth = 1;
    for (let angle = 0.2; angle <= 0.8; angle += 0.2) {
      ctx.beginPath();
      ctx.moveTo(0, by - 2 - breathe);
      ctx.lineTo(Math.cos(angle * Math.PI) * 13, by - 2 - breathe - Math.sin(angle * Math.PI) * 8);
      ctx.stroke();
    }
    ctx.restore();
  }
  ...
  static drawCrab(ctx, by, animTimer, direction, isMoving) {
    ...
    const flip = direction === 'left' ? -1 : 1;
    ctx.scale(flip, 1);
    const legWalk = isMoving ? Math.sin(animTimer * 0.3) * 3 : 0;
    const clawSnap = Math.sin(animTimer * 0.2) * 2;
    ...
    // 左步足
    ctx.beginPath();
    ctx.moveTo(-6, by + 4); ctx.lineTo(-14, by + 6 + legWalk); ctx.lineTo(-18, by + 12);
    ctx.moveTo(-5, by + 6); ctx.lineTo(-13, by + 9 - legWalk); ctx.lineTo(-16, by + 14);
    ctx.moveTo(-4, by + 8); ctx.lineTo(-11, by + 12 + legWalk); ctx.lineTo(-13, by + 16);
    // 右步足
    ctx.moveTo(6, by + 4); ctx.lineTo(14, by + 6 - legWalk); ctx.lineTo(18, by + 12);
    ctx.moveTo(5, by + 6); ctx.lineTo(13, by + 9 + legWalk); ctx.lineTo(16, by + 14);
    ctx.moveTo(4, by + 8); ctx.lineTo(11, by + 12 - legWalk); ctx.lineTo(13, by + 16);
    ...
    // 黄金坚硬蟹背甲 (梯形圆角厚甲)
    const shellGrad = ctx.createLinearGradient(-10, by, 10, by);
    shellGrad.addColorStop(0, '#b45309');
    shellGrad.addColorStop(0.5, '#f59e0b');
    shellGrad.addColorStop(1, '#b45309');
    ctx.fillStyle = shellGrad;
    ctx.beginPath();
    ctx.roundRect(-10, by - 2, 20, 12, 4);
    ctx.fill();
    ctx.strokeStyle = '#ffd700';
    ctx.lineWidth = 1.2;
    ctx.stroke();
    ...
    ctx.beginPath();
    ctx.arc(-3, by - 5, 1.6, 0, Math.PI * 2);
    ctx.arc(3, by - 5, 1.6, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = '#ffffff';
    ctx.beginPath();
    ctx.arc(-2.5, by - 5.5, 0.6, 0, Math.PI * 2);
    ctx.arc(3.5, by - 5.5, 0.6, 0, Math.PI * 2);
    ctx.fill();
    ...
    // 左巨钳
    ctx.beginPath();
    ctx.ellipse(-14, by - 4 + clawSnap, 6, 4, -0.4, 0, Math.PI * 2);
    ctx.fill();
    ctx.stroke();
    // 右巨钳
    ctx.beginPath();
  ...
  static drawShrimp(ctx, by, animTimer, direction, isMoving) {
    ...
    const flip = direction === 'left' ? -1 : 1;
    ctx.scale(flip, 1);
    const antennaWave = Math.sin(animTimer * 0.18) * 4;
    const tailKick = isMoving ? Math.sin(animTimer * 0.35) * 3 : 0;
    ...
    ctx.beginPath();
    ctx.moveTo(3, by - 12);
    ctx.quadraticCurveTo(10, by - 24 + antennaWave, 22, by - 26);
    ctx.moveTo(0, by - 12);
    ctx.quadraticCurveTo(-6, by - 22 - antennaWave, -18, by - 24);
    ctx.stroke();
    ...
    for (let seg = 0; seg < 4; seg++) {
      ctx.beginPath();
      ctx.ellipse(0, by - 4 + seg * 4 + tailKick * 0.2 * seg, 8 - seg * 1.2, 3.5, 0, 0, Math.PI * 2);
      ctx.fill();
    ...
    ctx.beginPath();
    ctx.moveTo(-5, by + 12 + tailKick);
    ctx.lineTo(5, by + 12 + tailKick);
    ctx.lineTo(8, by + 17 + tailKick);
    ctx.lineTo(0, by + 15 + tailKick);
    ctx.lineTo(-8, by + 17 + tailKick);
    ctx.closePath();
    ctx.fill();
    ...
    ctx.fillStyle = '#000000';
    ctx.beginPath();
    ctx.arc(3, by - 10, 1.5, 0, Math.PI * 2);
    ctx.arc(-3, by - 10, 1.5, 0, Math.PI * 2);
    ctx.fill();

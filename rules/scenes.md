...
### 4.5 白虎岭 (`baihuling`)
- **画面与生态**：灰绿枯草、朽骨碎石、紫灰湿洞构成独有地貌；怨灵骷髅使用骨甲与魂火专属模型，不再误画为天将。
- **主线阶段**：同一接引位置依次出现村姑、老妪、老翁三重画皮；三战分别保存为 `baihu_first_cleared`、`baihu_second_cleared`、`baihu_cleared`。第三战后妖王实体消失，宝象国开放。金仙收服规则仍以 `rules/gameplay.md` 为准。

### 4.6 宝象国 (`baoxiangguo`)
- **画面与角色**：王都草地偏秋金色；奎木狼持冷月宝刀，黄袍乌金甲与狼首星冠使用独立全身模型和立绘。国王、百花羞与妖王按主线阶段出场，战胜妖王后不再重刷。

### 4.7 陷空山·无底洞 (`wudidong` 官方高阶团队副本)
...
- 地图数据中出现的每一种瓦片均须有独立或明确复用的绘制分支。`demon_cave_wall` 为妖洞岩壁（灰紫／乌金裂隙）；`dirt_road` 复用古道土路纹理；`wuxing_seal` 为五行山金帖法阵。严禁落入纯黑默认方块。
- 同类瓦片可以按地图呈现不同风貌：刘家村草地用暖绿与小花，花果山草地和土路带桃花落英，陈塘关青石路偏海风灰并带零星贝纹。装饰依坐标稳定生成，不改变通行碰撞。
- 序章核心人物必须按当时身份绘制：太白金星为星官朝服、玉简与拂尘；嫦娥为广寒月轮、月白云裳与玉兔佩；卷帘大将为天庭御前乌金甲和镇殿杖。不得复用唐僧、七仙女或受贬后的沙悟净模型。
